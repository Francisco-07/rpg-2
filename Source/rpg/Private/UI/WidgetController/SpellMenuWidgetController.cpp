...
void USpellMenuWidgetController::BroadcastInitialValues()
{
	...
	SpellPointsChanged.Broadcast(GetRPGPS()->GetSpellPoints());
}
	...
	GetRPGASC()->AbilityStatusChanged.AddLambda([this](const FGameplayTag& AbilityTag, const FGameplayTag& StatusTag, int32 NewLevel)
{
	...
	GetRPGPS()->OnSpellPointsChangedDelegate.AddLambda([this](int32 SpellPoints)
{
...
void USpellMenuWidgetController::SpellGlobeSelected(const FGameplayTag& AbilityTag)
{
	...
	const int32 SpellPoints = GetRPGPS()->GetSpellPoints();
	FGameplayTag AbilityStatus;	
	
	const bool bTagValid = AbilityTag.IsValid();
	const bool bTagNone = AbilityTag.MatchesTag(GameplayTags.Abilities_None);
	const FGameplayAbilitySpec* AbilitySpec = GetRPGASC()->GetSpecFromAbilityTag(AbilityTag);
